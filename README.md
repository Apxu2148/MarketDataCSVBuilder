...
`catalog.csv` and `latest_snapshot.csv` also expose four append-only mapping-assistance fields: `source_underlying_symbol`, `underlying_symbol`, `contract_expiry`, and `underlying_liquidity_rank`. The first preserves a venue-native underlying identifier when available; `underlying_symbol` is only a normalized hint; expiry is an ISO date for dated contracts; and the liquidity rank orders READY series by average turnover inside the same source/market/underlying family. These fields are advisory and never alter `status=READY`, replace a downstream canonical mapping registry, or silently make an otherwise ineligible series tradable. Existing catalog columns and the original 53 latest-snapshot columns retain their prior positions; the new fields are appended.
...
For mapping metadata, MOEX discovery additionally reads venue `ASSETCODE` and `LASTTRADEDATE`. Known source families needed by APX regression checks are normalized conservatively as advisory hints (`GAZR`→`GAZP`, `SBRF`→`SBER`, `MIX`→`IMOEX`) while the original `ASSETCODE` is retained separately. Bybit uses `baseCoin` and, for dated `LinearFutures`, `deliveryTime`. Perpetual contracts have empty expiry.
